...
for x = 1:(length(bground1)-13);
 ...
 I = [xmid xleft xlmid xrmid xr xleft xleft2 xlmid2 xmid2 xrmid2 xr2]; 
    for complexx = 1:length(I);
        complexxi = ((I(complexx) *b)/(a+sum(I)));
        
        complexxitot = [complexxitot complexxi]; 
    end
    therealdeal = [therealdeal sum(complexxitot)];    
  ...
  needsa = (1:length(therealdeal))
  plot (needsa, therealdeal)
